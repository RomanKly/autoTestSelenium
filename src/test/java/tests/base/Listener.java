...
public class Listener implements TestWatcher {
	...
	public void testFailed(ExtensionContext context, Throwable cause){
		...
		try {
			FileUtils.copyFile(source, new File("build/report/tests/" + screenshotName + ".png"));
		} catch (IOException e) {
			LOGGER.info("Exception on saving screenshot");
			e.printStackTrace();
		}
		screenshotToAllure(ts);
	}

	@Attachment
	public byte[] screenshotToAllure(TakesScreenshot takesScreenshot){
